...
remote application. The token package should be the source of truth, not the
shell and not Zeroheight.
...
- derived TypeScript exports for tests and framework integrations;
- a PrimeNG preset mapped from the same token values.

The shell should load the generated token CSS and own active theme selection.
Theme state should be represented as stable classes or attributes on a shared
ancestor such as `html`, `body`, the shell root, or the Web Component host.

Each remote should consume the same token package so it can render correctly
inside the shell and outside the shell during local development, Storybook, and
remote-specific testing. Remotes may inherit token values from the shell at
...
The best consumption model is a shared, versioned token package that emits CSS
custom properties, metadata, derived TypeScript exports, and a PrimeNG preset.
The shell owns active theme context. Each remote consumes the same package so it
works both inside and outside the shell. DOM boundaries, overlays, and version
drift are the critical validation points.
