...
export const componentFindings: ComponentFinding[] = [
  ...
  },
  {
    ...
    summary:
      'Public API extraction was partial for four exported entries; all four now have complete '
      + 'documented inputs, outputs, or service methods and public types in the manifest.',
    evidence: [
...
export const componentRegistry = [
  ...
  entry({
    ...
    testStatus: 'complete',
    testFiles: ['apps/qa-remote/e2e/button-candidate.storybook.spec.ts'],
  ...
  entry({
    ...
    inputs: [
      ...
      { name: 'subheader', type: 'string', defaultValue: "''" },
    ],
  ...
  entry({
    id: 'ps-tag',
    tokenBoundary: 'provider-managed',
    findingIds: ['API-PARTIAL-001'],
    name: 'Tag', exportName: 'PublicTagComponent', selector: 'ps-tag',
    source: 'packages/ui-patterns/src/public-tag.component.ts',
    description: 'PrimeNG-backed status label wrapper.', provider: 'primeng',
    providerModules: ['primeng/tag'],
    publicApiStatus: 'complete',
    inputs: [
      { name: 'value', type: 'string', required: true },
      { name: 'tone', type: 'PublicTagTone', defaultValue: 'info' },
    ],
    publicTypes: ['PublicTagTone'],
  ...
  entry({
    ...
    findingIds: ['API-PARTIAL-001'],
    name: 'Toast Service', exportName: 'PublicToastService', selector: null,
    source: 'packages/ui-patterns/src/public-toast.service.ts',
    description: 'Public service API for creating governed toast messages via add(message: PublicToastMessage) and remove(id: number); messages() exposes the current queue as a readonly signal.',
    kind: 'service', provider: 'service',
    publicApiStatus: 'complete', publicTypes: ['PublicToastMessage', 'PublicToastSeverity'],
    storybookStatus: 'not-applicable', testStatus: 'partial',
