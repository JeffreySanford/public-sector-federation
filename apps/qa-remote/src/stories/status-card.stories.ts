...
export const Warning: Story = { args: { label: 'Approaching deadline', value: 42, detail: 'Response required within two business days', status: 'Needs attention', tone: 'warn' } };
export const Critical: Story = { args: { label: 'Overdue determinations', value: 9, detail: 'Escalation and documented resolution required', status: 'Action required', tone: 'danger' } };
...
@Component({ selector: 'status-card-matrix', standalone: true, imports: [PublicStatusCardComponent], template: `<main>@for(card of cards;track card.label){<public-status-card [label]="card.label" [value]="card.value" [detail]="card.detail" [status]="card.status" [tone]="$any(card.tone)" />}</main>`, styles: `main{display:grid;grid-template-columns:repeat(auto-fit,minmax(15rem,1fr));gap:1rem;max-width:72rem}` }) class StatusCardMatrix { cards=[{label:'Received',value:812,detail:'This month',status:'Normal',tone:'info'},{label:'Approved',value:694,detail:'85.5% approval rate',status:'On track',tone:'success'},{label:'Exceptions',value:37,detail:'Manual review required',status:'Watch',tone:'warn'},{label:'Overdue',value:9,detail:'Outside target',status:'Action',tone:'danger'}]; }
