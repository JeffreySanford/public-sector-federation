...
@Component({ selector: 'empty-action-proof', standalone: true, imports: [PublicEmptyStateComponent], template: `<ps-empty-state title="No saved filters" message="Save a filter to reuse it later." actionLabel="Create filter" (activated)="count=count+1" /><output aria-live="polite">Actions: {{count}}</output>` }) class EmptyActionProof { count=0; }
