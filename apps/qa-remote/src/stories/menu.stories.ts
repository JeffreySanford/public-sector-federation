...
export const GroupedDestructiveActions: Story = { args: { actions: [{ label: 'View record', icon: 'pi pi-eye' }, { label: '', separator: true }, { label: 'Delete record', icon: 'pi pi-trash', tone: 'destructive' }] } };
export const KeyboardInstructions: Story = { render: () => ({ props: { actions: standardActions }, moduleMetadata: { imports: [PublicMenuComponent] }, template: `<main><p>Tab to the trigger, press Enter, then use Arrow keys and Escape.</p><ps-menu label="Keyboard action menu" [actions]="actions" /></main>` }) };
