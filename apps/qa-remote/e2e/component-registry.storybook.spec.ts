...
test.describe('component manifest Storybook registry', () => {
  ...
  test('shows PrimeNG, native, composite, and service boundaries', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Provider boundaries', exact: true, level: 2 })).toBeVisible();
  ...
  test('renders external documentation readiness honestly', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Documentation readiness', exact: true, level: 2 })).toBeVisible();
