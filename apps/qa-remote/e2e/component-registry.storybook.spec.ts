...
test.describe('component manifest Storybook registry', () => {
  test('renders the full public registry overview', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Component Registry', exact: true })).toBeVisible();
  ...
  test('shows PrimeNG, native, composite, and service boundaries', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { name: 'Provider boundaries', level: 2, exact: true }),
    ).toBeVisible();
  ...
  test('states accessibility evidence without claiming conformance', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Accessibility evidence', exact: true })).toBeVisible();
  ...
  test('renders external documentation readiness honestly', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { name: 'Documentation readiness', level: 2, exact: true }),
    ).toBeVisible();
  ...
  test('renders UP Button promotion evidence and blockers', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'UP Button Candidate', exact: true })).toBeVisible();
