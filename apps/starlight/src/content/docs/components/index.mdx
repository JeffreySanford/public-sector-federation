...
import ComponentStatusTable from '../../../components/ComponentStatusTable.astro';
...
- **Lifecycle:** <StatusBadge status="active" /> and used in the reference applications.
...
- **Figma:** <StatusBadge status="pending-access" label="Pending access" /> and alignment review.
...
- **Lifecycle:** <StatusBadge status="active" />.
...
- **Lifecycle:** <StatusBadge status="active" /> and used in the reference remotes (`services-remote`, `admin-remote`, `reporting-remote`).
...
- **Figma:** <StatusBadge status="pending-access" label="Pending access" /> and design alignment review, same as the flagship trio.
...
The narrative above covers the six components with a complete flagship-model documentation page.
The full public export surface is larger — every entry below is read directly from the generated
manifest at build time, so this table always matches
[`packages/ui-patterns/generated/component-manifest.json`](https://github.com/JeffreySanford/public-sector-federation/blob/master/packages/ui-patterns/generated/component-manifest.json)
without anyone needing to hand-update it here.
